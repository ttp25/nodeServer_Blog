module.exports = (async function () {
    ...
    let flag = 0;
    let ArtNoList = []; // 货号数组
    let laterArtNoList = []; // 上一次定时器的货号数组

    setInterval(async () => {
        let ArtList;
        await getCargo().then(res => { ArtList = res; });

        ArtList.forEach(item => {
            if (flag) {
                ArtNoList.push(item.ArtNo);
            } else {
                laterArtNoList.push(item.ArtNo);
            }
        });

        flag++;

        console.log('ArtNoList', ArtNoList);
        console.log('laterArtNoList', laterArtNoList);

        if (ArtNoList.length > 0 && laterArtNoList.length > 0) {
